...
public class SupplyDemandAdjustmentRule extends PricingEngineRuleBase {
	...
	public void applyRule(ProductPricing productPricing) throws Exception {
		...
		if (null != productPricing.getProduct() && null != productPricing.getProduct().getSupply()
				&& null != productPricing.getProduct().getDemand()) {
			if (productPricing.getProduct().getDemand() == Demand.HIGH
					&& productPricing.getProduct().getSupply() == Supply.LOW) {
				// Sell at 5% More
				productPricing.setRecommendedPrice(productPricing.getRecommendedPrice()
						.add(CalculationUtil.multiply(productPricing.getRecommendedPrice(), FIVE_PERCENT)));
			} else if (productPricing.getProduct().getDemand() == Demand.LOW
					&& productPricing.getProduct().getSupply() == Supply.HIGH) {
				// Sell at 5% Less
				productPricing.setRecommendedPrice(productPricing.getRecommendedPrice()
						.subtract(CalculationUtil.multiply(productPricing.getRecommendedPrice(), FIVE_PERCENT)));
			} else if (productPricing.getProduct().getDemand() == Demand.LOW
					&& productPricing.getProduct().getSupply() == Supply.LOW) {
				// Sell at 10% More
				productPricing.setRecommendedPrice(productPricing.getRecommendedPrice()
						.add(CalculationUtil.multiply(productPricing.getRecommendedPrice(), TEN_PERCENT)));
			}
		}
